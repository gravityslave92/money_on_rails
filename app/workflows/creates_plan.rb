class CreatesPlan
  attr_accessor :remote_id, :name, :price_cents, :plan,
                :interval, :tickets_allowed, :ticket_category

  def initialize(remote_id:, name:, price_cents:,
                 interval:, tickets_allowed:, ticket_category:)
    @remote_id = remote_id
    @name = name
  ...
  def run
    remote_plan = Stripe::Plan.create(
      ...
      currency: "usd", interval: interval, name: name
    )
